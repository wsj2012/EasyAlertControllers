...
public enum EasyAlertActionStyle: Int {
    case `default`
    case cancel
    case destructive
}

public enum EasyAlertControllerStyle: Int {
    case actionSheet
    case alert
}

open class EasyAlertAction: NSObject, NSCopying {
    
    open var title: String
    open var style: EasyAlertActionStyle
    var handler: ((EasyAlertAction?) -> Void)!
    ...
    required public init(title: String, style: EasyAlertActionStyle, handler: ((EasyAlertAction?) -> Void)!) {
        self.title = title
        self.style = style
...
class EasilyAlertController: UIViewController, UITextFieldDelegate, UIViewControllerTransitioningDelegate {
    ...
    // overlayView
    fileprivate var overlayView = UIView.init()
    ...
    // AlertView
    open var alertView = UIView.init()
    ...
    open var buttonFont: [EasyAlertActionStyle : UIFont] = [
    ...
    open var buttonTextColor: [EasyAlertActionStyle : UIColor] = [
        .default : UIColor.white,
        .cancel  : UIColor.white,
        .destructive  : UIColor.white
    ]
    open var buttonBgColor: [EasyAlertActionStyle : UIColor] = [
    ...
    open var buttonBgColorHighlighted: [EasyAlertActionStyle : UIColor] = [
    ...
    public convenience init(title: String?, message: String?, preferredStyle: EasyAlertControllerStyle) {
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        if !isAlert(), cancelButtonTag != 0 {
            let tapGesture = UITapGestureRecognizer(target: self, action: #selector(handleContainerViewTapGesture(_:)))
    ...
    open func layoutView(_ presenting: UIViewController?) {
        ...
        if (isAlert() && buttons.count == 2) {
            ...
            for button in buttons {
                let action = actions[button.tag - 1] as! EasyAlertAction
            ...
            if (cancelButtonTag != 0) {
                ...
                let action = actions[cancelButtonTag - 1] as! EasyAlertAction
                button.titleLabel?.font = buttonFont[action.style]
                button.setTitleColor(buttonTextColor[action.style], for: .normal)
                button.setBackgroundImage(createImageFromUIColor(buttonBgColor[action.style]!), for: .normal)
                button.setBackgroundImage(createImageFromUIColor(buttonBgColorHighlighted[action.style]!), for: .highlighted)
    ...
    @objc func handleAlertActionEnabledDidChangeNotification(_ notification: Notification) {
        for i in 0..<buttons.count {
            buttons[i].isEnabled = (actions[i] as! EasyAlertAction).enabled
        }
    }
    ...
    @objc func buttonTapped(_ sender: UIButton) {
        ...
        let action = actions[sender.tag - 1] as! EasyAlertAction
    ...
    //MARK: - Public Methods
    open func addAction(_ action: EasyAlertAction) {
        // Error
        if (action.style == .cancel) {
            for ac in actions as! [EasyAlertAction] {
                if (ac.style == .cancel) {
                    let error: NSError? = nil
                    NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format:"EasyAlertController can only have one action with a style of EasyAlertActionStyleCancel", arguments:getVaList([error ?? "nil"]))
                    return
                }
            }
        }
    ...
    open func addTextFieldWithConfigurationHandler(_ configurationHandler: ((UITextField?) -> Void)!) {
        ...
        // You can add a text field only if the preferredStyle property is set to EasyAlertControllerStyle.Alert.
        if (!isAlert()) {
            let error: NSError? = nil
            NSException.raise(NSExceptionName(rawValue: "NSInternalInconsistencyException"), format: "Text fields can only be added to an alert controller of style EasyAlertControllerStyleAlert", arguments:getVaList([error ?? "nil"]))
            return
        }
        if (textFields == nil) {
            textFields = []
    ...
    open func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        ...
        return EasyAlertAnimation(isPresenting: true)
    }
    
    open func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return EasyAlertAnimation(isPresenting: false)
    }
}
